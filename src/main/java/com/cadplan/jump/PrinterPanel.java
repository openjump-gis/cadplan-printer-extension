...
public class PrinterPanel extends JPanel
    implements Printable, Pageable, Comparator<Furniture> {
  boolean debug = false;
   ...
   *
   * @param drawScale whether a scale should be draw or not
   */
   ...
   * //TODO qualityOption does not seem to be used ??
   * @param qualityOption
   */
   ...
   * sets the title item
   *
   * @param title title furniture to set
   */
   ...
   * sets the scale item
   *
   * @param scaleItem scale furniture to set
   */
   ...
   * sets the note item
   *
   * @param note note furniture to set
   */
   ...
   * paints the image
   *
   * @param gp graphic context
   */
  public void paint(Graphics gp) {
    //boolean debug = true;
    int xb = (int) (bounds.getWidth() * 1000.0 * pixelUnit / scale); //xsize;
    int yb = (int) (bounds.getHeight() * 1000.0 * pixelUnit / scale); //ysize;
    int xoffs = xoff;
    int yoffs = yoff;
...
//    else
    ...
    else if (printingMode == 0)  // new isa quality option
    {
      ...
      try {
        ...
        //List<Layer> layerCollection = context.getLayerViewPanel().getLayerManager().getLayers();
        ...
        for (int j = allCategories.size() - 1; j >= 0; j--) {
          Category cat = allCategories.get(j); //j.next();
          ...
          for (int i = allLayerables.size() - 1; i >= 0; i--) {
            ...
            Layerable layerable = allLayerables.get(i); // i.next();
            if (layerable.isVisible()) {
              ...
              } else    // for all others (eg images)
              {
                ...
                if (layerable instanceof WMSLayer) scaleGraphics = false;
...
//        }
    ...
    itemsForPrinting = new Vector<>();
    if (border.show) itemsForPrinting.addElement(border);
    for (int i = 0; i < borders.size(); i++)
      if (borders.elementAt(i).show) itemsForPrinting.addElement(borders.elementAt(i));
    ...
    for (int i = 0; i < imageItems.size(); i++)
      if (imageItems.elementAt(i).show) itemsForPrinting.addElement(imageItems.elementAt(i));
    //System.out.println("Number of printable items: "+ itemsForPrinting.size());
    itemsForPrinting.sort(this);
...
/*
  ...
  public int compare(Furniture item1, Furniture item2) {
    if (item1.layerNumber < item2.layerNumber) return -1;
    else if (item1.layerNumber > item2.layerNumber) return 1;
    else return 0;
  }

  private LayerViewPanel createLayerPanel(LayerManager layerManager, final Throwable[] throwable) {

    return new LayerViewPanel(layerManager,
   ...
   * sets the PageFormat
   *
   * @param pageFormat the page format to set
   */
...
//            ysize = yoff + visRect.height;
    ...
    drawingScale = bounds.getWidth() * 1000.0 * pixelUnit / scale / ((double) visRect.width); // ratio of required size to actual size - temp fix
   ...
   * @throws IndexOutOfBoundsException if the index is >= than numPages
   */
   ...
   * @param pageIndex  index of page to print
   * @return PAGE_EXISTS
   */

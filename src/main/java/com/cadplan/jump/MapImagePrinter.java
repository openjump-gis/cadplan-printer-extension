...
public class MapImagePrinter extends Component {
  ...
  public void createImage(Graphics2D graphics, int xoff, int yoff) {
    ...
    List<Layer> allLayers = context.getLayerViewPanel().getLayerManager().getLayers();
    List<Category> allCategories = context.getLayerViewPanel().getLayerManager().getCategories();
    ...
    if (includeImages) {
      for (Category cat : allCategories) {
        List<Layerable> allLayerables = cat.getLayerables();
        if (debug) System.out.println("Category: " + cat.getName() + "  size=" + allLayerables.size());

        for (Layerable layerable : allLayerables) // extract all visible raster layers
        {
          try {
            Class.forName("org.openjump.core.rasterimage.RasterImageLayer");
            if (layerable instanceof RasterImageLayer) {
              boolean isVisible = layerable.isVisible();
              if (isVisible) rasterLayers.add(layerable);
              if (debug)
                System.out.println("Layer:" + layerable.getName() + "        Layer is Raster***********  visible:" + isVisible);
            }
          ...
          if (!(layerable instanceof Layer)) {
            if (debug)
              System.out.println("External Rendering image layer: " + layerable.getName() +
									"  type:" + layerable.getClass().toString());
            if (layerable.isVisible()) rasterLayers.add(layerable);
          }
        }
      }
      for (int jj = rasterLayers.size() - 1; jj >= 0; jj--) // scan in reverse order
      {
        Layerable layerable = rasterLayers.get(jj);
        ...
        if ((layerable instanceof WMSLayer)) {
          try {
            Image image = ((com.vividsolutions.jump.workbench.model.WMSLayer) layerable).createImage(context.getLayerViewPanel());
            ...
            transparencyLevel = 1.0 - ((com.vividsolutions.jump.workbench.model.WMSLayer) layerable).getAlpha() / 255.0;
          ...
          bimage = ((RasterImageLayer) layerable).createImage(context.getLayerViewPanel());
          imageEnvelope = ((RasterImageLayer) layerable).getWholeImageEnvelope();
          transparencyLevel = ((RasterImageLayer) layerable).getTransparencyLevel();
          imageX = ((RasterImageLayer) layerable).getXOffset();
          imageY = ((RasterImageLayer) layerable).getYOffset();
        }
    ...
    List<Layer> layerCollection = context.getLayerViewPanel().getLayerManager().getVisibleLayers(true);


    int count = 0;
    Layer[] layerArray = layerCollection.toArray(new Layer[0]);
    for (int i = layerArray.length - 1; i >= 0; i--) {
      Layer layer = layerArray[i]; //i.next();
      ...
      String desc = layer.getDescription();
			rd.vertexSize = layer.getVertexStyle().getSize();
      rd.vertexStyle = layer.getVertexStyle();
			rd.showVertex = layer.getVertexStyle().isEnabled();
			rd.labelColor = layer.getLabelStyle().getColor();
      ...
      Collection<BasicStyle> themeStyles = null;
      Map<Object, BasicStyle> themeMap;
      ...
      BasicStroke lineStroke = basicStyle.getLineStroke();
      rd.lineWidth = lineStroke.getLineWidth() * drawFactor;
      float[] dash = lineStroke.getDashArray();
      try {
        for (int id = 0; id < dash.length; id++) dash[id] = dash[id] * (float) drawFactor;
      } catch (NullPointerException ignored) {

      }
      float dashPhase = lineStroke.getDashPhase() * (float) drawFactor;
      float miterLimit = lineStroke.getMiterLimit();
      ...
      while (k.hasNext()) {
        ...
        if (featureIsImage) {
          ...
          if (themeStyles != null) {
            ...
            Iterator<Object> ki = mapKeys.iterator();
            ...
            while (ki.hasNext()) {
              ...
              if (!isRange) {
                ...
                String themeAttributeValueString;
                try {
            ...
            Iterator<BasicStyle> si = themeStyles.iterator();
            kk = 0;
            boolean found = false;
            while (si.hasNext() && !found) {
              BasicStyle style = si.next();
            ...
            if (!found) {
              ...
              if (debug) System.out.println("Theming style not found for index:" + index);
            } else {
          ...
          if (type.equals("MultiPoint") || type.equals("Point")) {
            try {
              ...
              int numGeom = 1;
              try {
                numGeom = inter.getNumGeometries();
              } catch (Exception ignored) {
              ...
              if (inter != null && numGeom > 0) {
                for (int n = 0; n < numGeom; n++) {
                  ...
                  drawPoint(graphics);
                  if (labelValue != null) {
										rd.labelHeight = getAttribute(feature, labelHeight);
										rd.labelAngle = getAttribute(feature, labelAngle);
                    rd.lineAngle = 0.0;
                    rd.x = coords[0].x;
                    rd.y = coords[0].y;
                    drawLabel(graphics);
  ...
  private int pY(double y) {
    return yoff + ysize - (int) Math.round((y - envelope.getMinY()) * scale);
  }
  ...
  private void drawPoint(Graphics2D g) {
    ...
    if (rd.vertexStyle.isEnabled()) {
      try {
        Class.forName("com.cadplan.jump.utils.VertexStyler"); // test if VertexSymbols plugin is installed
      ...
      } catch (ClassNotFoundException ignored)  // VertexSymbols plugin not installed
      {
  ...
  private void drawPolygon(Graphics2D g) {
    ...
    float x0 = 0.0f, x, xp = 0.0f;
    float y0 = 0.0f, y, yp = 0.0f;
    double startAngle, endAngle, segmentAngle;
    rd.lineWidth = ((BasicStroke) rd.lineStroke).getLineWidth();
    int index = 0;
    Coordinate[] coords = ((Polygon) rd.polygon).getExteriorRing().getCoordinates();
    int numberHoles = ((Polygon) rd.polygon).getNumInteriorRing();
    GeneralPath path = new GeneralPath();
    float xs = 0.0f;
    float ys = 0.0f;
    ...
    if (numberHoles > 0) {
      ...
      for (int n = 0; n < numberHoles; n++) {
        index = 0;
        Coordinate[] hcoords = ((Polygon) rd.polygon).getInteriorRingN(n).getCoordinates();
  ...
  private void drawXY(Graphics2D g, float x, float y, double angle, int index, double lineWidth) {
    g.setFont(vertexFont);
    g.setColor(vertexFontColor);
    String note = format(rd.thisVertex.getX()) + "," + format(rd.thisVertex.getY());
    g.drawString(note, x, y);
  }
  ...
  private void drawMetrics(Graphics2D g, float x, float y, double angle, double lineWidth) {
    ...
    String note = format(len) + "/" + formatAngle(Math.toDegrees(bearing)) + "°";
    g.drawString(note, x, y);

...
public class LayerLegend extends Furniture {
  ...
  public void updateLegend(PlugInContext context) {
    ...
    while (i.hasNext()) {
      ...
      Collection<String> keyValues = null;
      if (themeStyle.isEnabled()) {
        Map<Object, BasicStyle> themeMap = themeStyle.getAttributeValueToBasicStyleMap();
        Map<Object, String> labelMap = themeStyle.getAttributeValueToLabelMap();
        ...
        if (debug) System.out.println("lableMap:" + labelMap);
        if (debug) System.out.println("mapKeys:" + keyValues);
  ...
  public void paint(Graphics g, double scale, double globalScale, int printingMode) {
    ...
    for (int i = 0; i < legendItems.size(); i++) {
      ...
      if (element.include) {
        ...
        Iterator<String> itm = element.keyValues.iterator();
        int count = 0;

        while (it.hasNext()) {
          BasicStyle style = it.next();
          //keyMap = itm.next();
          String keyName = itm.next();
